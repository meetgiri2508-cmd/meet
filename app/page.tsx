...
type Clip={start:number;end:number;title:string;hookScore:number;reason:string;caption?:string};
...
export default function Home(){
 ...
 async function run(){
  if(!source.trim()){setStatus('Add a video URL or source first.');return;}
  setBusy(true); setClips([]); setStatus('Agent is analyzing the source…');
  try{
   ...
   setClips(data.clips||[]); setStatus(`${data.clips?.length||0} clip candidates generated.`);
 ...
 return <main style={{minHeight:'100vh',padding:'48px 24px',fontFamily:'system-ui',background:'linear-gradient(135deg,#080b16,#17102b)',color:'#fff'}}><div style={{maxWidth:1100,margin:'auto'}}>
  ...
  <p style={{fontSize:19,opacity:.72,maxWidth:720}}>Analyze authorized long-form content, find high-retention moments, generate captions and prepare clips for YouTube Shorts and TikTok.</p>
  <section style={{marginTop:36,padding:24,border:'1px solid #ffffff18',borderRadius:24,background:'#ffffff08'}}>
   <label style={{display:'block',fontWeight:700}}>Source URL / media identifier</label><div style={{display:'flex',gap:12,marginTop:12,flexWrap:'wrap'}}><input value={source} onChange={e=>setSource(e.target.value)} placeholder='YouTube URL or uploaded source ID' style={{flex:'1 1 500px',padding:16,borderRadius:12,border:'1px solid #ffffff20',background:'#070914',color:'#fff',fontSize:15}}/><button disabled={busy} onClick={run} style={{padding:'14px 24px',border:0,borderRadius:12,fontWeight:800,opacity:busy?.6:1}}>{busy?'Running agent…':'Create clips'}</button></div>
  ...
  {clips.length>0&&<><h2 style={{marginTop:36}}>AI-selected moments</h2><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(250px,1fr))',gap:16,marginTop:14}}>{clips.map((c,i)=><article key={`${c.start}-${i}`} style={{padding:20,borderRadius:18,border:'1px solid #ffffff18',background:'#ffffff08'}}><div style={{height:150,borderRadius:14,background:'linear-gradient(160deg,#292d4a,#080a12)',display:'grid',placeItems:'center',fontSize:34}}>▶</div><h3>{c.title}</h3><small>{Math.round(c.start)}s–{Math.round(c.end)}s • Hook score {c.hookScore}/100</small><p style={{opacity:.68,fontSize:14}}>{c.reason}</p><button style={{marginTop:8,padding:'10px 13px',borderRadius:10,border:'1px solid #ffffff22',background:'transparent',color:'#fff'}}>Queue render</button></article>)}</div></>}
