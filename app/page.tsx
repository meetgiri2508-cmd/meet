...
export default function Home(){
 const [source,setSource]=useState(''); const [transcript,setTranscript]=useState(''); const [file,setFile]=useState<File|null>(null); const [status,setStatus]=useState(''); const [clips,setClips]=useState<Clip[]>([]); const [busy,setBusy]=useState(false);
 ...
 return <main style={{minHeight:'100vh',padding:'48px 24px',fontFamily:'system-ui',background:'linear-gradient(135deg,#080b16,#17102b)',color:'#fff'}}><div style={{maxWidth:1100,margin:'auto'}}>
  ...
  <p style={{fontSize:19,opacity:.72,maxWidth:720}}>Upload authorized long-form content or provide a direct video URL. The agent finds moments, renders vertical clips and stores finished MP4s in R2.</p>
  ...
  <section style={{marginTop:40,display:'flex',gap:10,flexWrap:'wrap',opacity:.75}}>{['AI clip selection','R2 source storage','9:16 rendering','Video previews','YouTube Shorts','TikTok','Analytics learning'].map(x=><span key={x} style={{padding:'8px 12px',border:'1px solid #ffffff18',borderRadius:999}}>{x}</span>)}</section>
 </div></main>
}