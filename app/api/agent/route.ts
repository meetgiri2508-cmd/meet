...
import { buildClipPlan, clipAgent, TranscriptSegment } from '../../../../lib/agent';
...
export async function POST(req: Request) {
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      count = Math.min(20, Math.max(1, Number(form.get('count') || 5)));
    ...
    return NextResponse.json({ ok:true, plan, transcript, segments, clips });
  } catch (error) {
